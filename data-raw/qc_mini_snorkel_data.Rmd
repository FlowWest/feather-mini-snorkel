...
There are `r is.na(joined_fish_obs_2002$date) |> sum()` NA values- TODO:
do we want to remove these NA date values?
...
ggplot() +
  ...
  geom_point(data = anti_join_2001_data, aes(x = date, y = count, color = 'Data that is different in the 2002 dataset'), alpha = 0.5, size = 4) + 
  theme_minimal()+
...
-   2001 dataset goes from `r min(joined_fish_obs$date)` to
    `r max(joined_fish_obs$date)`

-   new 2001/2002 dataset goes from
