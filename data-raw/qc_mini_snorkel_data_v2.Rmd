...
This markdown explores the data that was provided for mini snorkel data for Feather River in the database: `minisnorkdb3.mdb`. Previously, a database was provided that only contained 2001 data (`MiniSnorkelDTB.mdb`). Upon exploration and comparison between this new database and the old database, it was decided that we will move forward using this database for publishing on EDI. For details on the `MiniSnorkelDTB` database visit: `qc_mini_snorkel_data.Rmd` and for details on the database comparison, visit: `database_qc.Rmd`. 
...
survey_locations <- all_fish_data |> 
  ...
  mutate(weather = tolower(weather),
         ...
         location_revised = case_when(grepl("big hole", location) ~ "big hole",
                              ...
                              grepl("goose", location) ~ "goose riffle", # TODO: is this a new location? 
                              grepl("aleck", location) ~ "aleck riffle", # TODO: is this a new location? 
                              ...
                              grepl("mcfarland", location) ~ "mcfarland riffle",
                              grepl("macfarland", location) ~ "mcfarland riffle",
                              grepl("trailer park", location) ~ "trailer park riffle",
                              grepl("trialer", location) ~ "trailer park riffle",
                              ...
                              grepl("hatchery riffle", location) ~ "hatchery riffle", # this wasn't in the 2001 data
                              grepl("hatchery ditch", location) ~ "hatchery ditch", # this wasn't in the 2001 data
                              T ~ location)) |> 
...
sort(unique(survey_locations$location_revised))
```
