...
location_data_updated <- survey_locations |> 
  ...
  mutate(channel_location = case_when(location == "shallow riffle" ~ "HFC",
...
Location reconciliation that still needs to be done for 2002 data:

-   For 2002 shallow riffle locations there are channel types that
    include `mainchannel`and `mainchannel branch`. These channel types
    don't exist in the 2001 lookup so there are NAs. To resolve, we can
    receive the coordinates for `mainchannel` and `mainchannel branch`
    or change the channel type to `side channel`

-   All remaining lat and longs are related to cox riffle and gateway
    riffle. We will need these coordinates and whether they are LFC/HFC
