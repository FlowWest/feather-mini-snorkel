...
The location names and coordinates were reconciled for the 2001 data (see
`qc_mini_snorkel_data.Rmd`). 

Here we will apply the same logic to reconcile names so that 2002 can be cleaned as well.

We will then clean the coordinates for 2002.

```{r}
# create fish obs table
...
# create locations table with logic to clean names
survey_locations <- all_fish_data |> 
  ...
  mutate(weather = tolower(weather),
         ...
         location_revised = case_when(grepl("big hole", location) ~ "big hole",
                                      ...
                                      grepl("alec", location) ~ "aleck riffle", 
...
The following code cleans the coordinates for 2002 as 2001 had previously been cleaned.

```{r, include = F}
# filter for 2002 location data to clean
locations_2002 <- survey_locations |> 
  filter(year(date) == 2002)
# convert gps coordinates for 2002 data where we are able to
temp_df <- locations_2002 |> 
  filter(!location_table_id %in% c(288,187,259,258,182,289,261,263,260,287,249,250,190,232,161,231,180,233,234)) |> # filter out rows that have wonky format and wont work
  filter(!is.na(gps_coordinate)) |> 
  select(location_table_id, gps_coordinate)

# function for converting to lat/long
convert_gps <- function(coordinate) {
# Split the coordinate into components
parts <- strsplit(coordinate, " ")[[1]]

# Extract components
lat_direction <- parts[1]
lat_degrees <- as.numeric(parts[2])
lat_minutes <- as.numeric(parts[3])

lon_direction <- parts[4]
lon_degrees <- as.numeric(parts[5])
lon_minutes <- as.numeric(parts[6])
...
if (lat_direction == "S") latitude <- -latitude
if (lon_direction == "W") longitude <- -longitude

list(latitude = latitude, longitude = longitude)
}
# Convert gps to latitude and longtidue
df <- temp_df %>%
  rowwise() %>%
  mutate(
    result = list(convert_gps(gps_coordinate)),
    latitude = result$latitude,
    longitude = result$longitude
  ) %>%
  select(-result)

# Find remaining 2002 rows that need to fix
# For these remaining ones we will use a coordinate for that location previously cleaned for 2001
locations_2002_to_fix <- full_join(locations_2002, df) |> 
...
# create a location lookup using the 2001 reconciled locations that can be used for 2002 data that have messy coordinates
lookup <- survey_locations_2001 |> 
  select(location, longitude, latitude, channel_location) |> 
  distinct(location, .keep_all=T) # we need one location, not multiple or will end up with duplicates

location_data_updated <- locations_2002_to_fix |>
  select(-c(latitude, longitude)) |>
  left_join(lookup) |> # fix messy 2002 coordinates by finding coordinate from 2001 same location
  mutate(coordinate_method = "assigned based on similar location from 2001") |>
  bind_rows(
      full_join(locations_2002, df) |> # add the 2002 converted lat/long
      filter(!is.na(latitude) & !is.na(longitude)) |>
      left_join(lookup |>
                  select(location, channel_location))
  ) |>
  mutate(coordinate_method = "reported in database") |>
  bind_rows(survey_locations_2001) |> # add on the 2001 survey locations
  arrange(location_table_id) |>
  # clean remaining locations
  mutate(
    coordinate_method = case_when(
      location %in% c("cox riffle", "gateway riffle") &
        is.na(latitude)  ~ "assigned based on similar location",
      T ~ coordinate_method
    ),
    latitude = case_when(
      location == "cox riffle" &
        is.na(latitude) ~  39.33564114789087,
      # based off of gps_coordinates
      location == "gateway riffle" &
        is.na(latitude) ~ 39.45696577928121,
      T ~ latitude
    ),
    longitude = case_when(
      location == "cox riffle" &
        is.na(longitude) ~ -121.63215000396984,
      # based off of gps_coordinates
      location == "gateway riffle" &
        is.na(longitude) ~ -121.62560364814165,
      location_table_id == 168 ~ -121.61602,
      location_table_id == 283 ~ -121.60900,
      T ~ longitude
    ),
    ...
    channel_type = case_when(location == "trailer park" ~ "mainchannel", T ~ channel_type) # this removes 1 NA but is consistent with all other trailer parks
  ) |> 
  select(-gps_coordinate)

# QC
# length(unique(all_fish_data$p_dat_id))
